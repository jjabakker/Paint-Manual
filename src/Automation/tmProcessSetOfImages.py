...
from src.Automation.tmProcessSingleImage import process_single_image
...
def process_all_images(image_directory):
    try:
        quality_criteria_file = "/Users/hans/Trackmate Library/Quality criteria.xlsx"
    ...
    # ------------------------------------------------------
    # Loop through all the images in the selected directory
    # ------------------------------------------------------

    all_dirs = os.listdir(image_directory)
    all_dirs.sort()
    for d in all_dirs:

        # There may already be files in the directory. Ignore anything but directories
        if os.path.isfile(image_directory + os.sep + d):
            continue

        # Create the individual results files containing the data for the selected cells
        image = image_directory + os.sep + d
        print('\nProcessing directory: ', image, "\n")
        process_single_image(image, d)
    ...
    for d in all_dirs:
        ...
        qfile = image_directory + os.sep + d + os.sep + d + '-results.xlsx'
    ...
    write_formatted_excel(image_directory, df_merged, qdf)


def PostProcessing():
    root = Tk()
    root.withdraw()
    image_directory = askdirectory(title='Select Folder')  # shows dialog box and return the path

    process_all_images(image_directory)

    results_file = image_directory + os.sep + "Full Results.xlsx"
    prepare_graphpad(results_file)
